...
数据真源 = 轴测图POC/ 活几何 + 家具 (基线即由 build.py D 从此产出),
...
POC = os.path.join(REPO, "轴测图POC")
BASELINE = os.path.join(REPO, ".phase0-baseline")

from floorplan_core import axon, geometry  # noqa: E402 (引擎库单一真源)

HOUSE = "D"
GEOM_JSON = os.path.join(POC, f"geometry-{HOUSE}户型.json")
FURN_JSON = os.path.join(POC, f"furniture-{HOUSE}户型.json")
