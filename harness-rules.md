...
**⚠ 本项目例外：进度类文件推 `main` 会触发完整 build+deploy** —— `deploy.yml` **没有** paths-ignore，任何 push `main` 都会重建镜像并重启生产容器，纯文档/状态改动也不例外（阅天府 calib-z-b1 实证：连推三次文档改动，每次都触发了完整部署）。故状态/记忆文件应尽量**随批次 PR 一起走**，避免单独推。
