...
MIN_CORRESPONDENCES = 10     # spec §D1 硬性
...
def cmd_points(args) -> int:
    ...
    pts = WP.candidates(G, args.room)
    if not pts:
        print(f"房间 {args.room} 无候选点（检查 room id）", file=sys.stderr)
        return 2
    out = {
        "room_id": args.room,
        "coplanar_warning": not WP.non_coplanar(pts),
...
def cmd_build(args) -> int:
    ...
    if len(corr) < MIN_CORRESPONDENCES:
        print(f"✗ 只有 {len(corr)} 个对应，spec §D1 要求 >= {MIN_CORRESPONDENCES}。"
    ...
    try:
        ...
        with Image.open(photo) as im:
            img_wh = list(im.size)
    except Exception:  # noqa: BLE001 - 夹具不该因读不到图就失败，宽高可由标注给
        img_wh = None
    ...
    print(f"对应 {len(corr)} 个 | 拟合中位 {fixture['fit']['median_px']}px | "
