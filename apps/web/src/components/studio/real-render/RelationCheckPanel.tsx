...
// 关系布置验收透出 (render-relation-b1, 与 AutoCheckPanel 并列互不相干): relation_check 仅
// strategy=relational 记录有; 旧记录/softref/geometry_lock 缺省 (undefined) 不展示。
// 软门语义: 不折叠不弱化大图, 只如实列出逐条核对结果供用户判断 (uncertain 不算 fail,
// 背景保真只分级不做门)。
...
// relational 档验收结果面板: relation_pass 徽章 + 逐条 checks + 背景保真 + 修正轮数。
export function RelationCheckPanel({ record }: { record: RenderRecord }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-gray-200 p-3 dark:border-white/10">
      ...
      {!rc.background_preserved && rc.background_issues.length > 0 && (
