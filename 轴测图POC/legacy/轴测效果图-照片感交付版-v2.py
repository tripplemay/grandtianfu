...
OUT = "/Users/yixingzhou/project/grandtianfu/轴测图POC/轴测效果图-照片感交付版-v2.svg"
SHOW_LABELS = False
...
for x, y, w, h, fill in furn:
    ...
    if 690 <= x <= 1040 and 980 <= y <= 1305 and ty in ("sofa", "ctable"):
        continue
    # #4 生活阳台 → 显式洗衣区（手工层重画），跳过旧"洗烘塔"块
    if 1005 <= x <= 1215 and 265 <= y <= 490:
        continue
    # #6 主卧床+床头柜由手工层锁向重画，跳过自动块
    if 1590 <= x <= 1815 and 1070 <= y <= 1310 and ty == "bed":
        continue
...
# 7. 客厅：沙发背朝西、面朝东；在沙发西侧背后增加酒柜。此处用单一清晰沙发块，减少 AI 误读。
...
add_box(710, 1015, 820, 1250, 0, 720, "#b07a4e", key=1860)       # 主沙发，背西面东
add_top_detail(735, 1040, 805, 1225, 725, "#c08a5d", 0.9, key=1865)
add_box(890, 1080, 1015, 1195, 0, 390, "#d8c9ad", key=2110)      # 东侧茶几，明确不是沙发南侧家具
add_top_detail(898, 1088, 1007, 1187, 395, "#f1eadc", 0.95, key=2115)

# 5. 生活阳台 = 洗衣区：洗烘一体塔(双圆舱门) + 水槽柜 + 上方吊柜（替代旧"洗烘塔"色块，避免被读成衣柜）
add_box(1022, 282, 1090, 362, 0, 1820, "#ededee", key=1300)          # 洗烘一体塔(白色家电)
for _zc in (470, 1160):                                              # 南面双圆舱门
    _p = proj(1056, 362, _zc)
    emit(1306, f'<circle cx="{_p[0]:.1f}" cy="{_p[1]:.1f}" r="12" fill="#9fb6c4" stroke="#6f8a99" stroke-width="2"/>')
add_box(1110, 282, 1200, 345, 0, 850, "#d9d0bf", key=1330)           # 水槽柜
add_top_detail(1122, 292, 1190, 338, 858, "#b8d7e4", 0.85, key=1335) # 不锈钢水槽盆
add_box(1022, 282, 1200, 312, 1500, 1900, "#cfc4b2", key=1640)       # 上方吊柜

# 6. 主卧：床头锁定靠东墙(SE实墙)、床尾朝西侧休闲区（复用 m_bed 强制 side='E'）+ 床头柜 + 西侧贵妃榻
shadow(1608, 1120, 1810, 1302, 1709, 1211)
piece(m_bed(1608, 1120, 1810, 1302, 'E', "#d8c9ad"), 1709, 1211)
add_box(1766, 1072, 1810, 1112, 0, 470, "#8a633e")                   # 床头柜(北)
add_box(1766, 1308, 1810, 1348, 0, 470, "#8a633e")                   # 床头柜(南)
shadow(1285, 1140, 1390, 1310, 1335, 1225)
add_box(1285, 1140, 1390, 1310, 0, 400, "#3d5440")                   # 西侧贵妃榻/躺椅座(墨绿丝绒)
add_box(1285, 1140, 1318, 1310, 400, 780, "#34492f")                 # 躺椅靠背(西端)
add_top_detail(1295, 1150, 1382, 1300, 405, "#46604a", 0.95)         # 坐垫面
