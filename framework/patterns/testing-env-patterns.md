...
## 14. 「重构应等价」类改动，逐字节输出对照是金标准，比「测试跑绿」强一个量级（v1.0.7 — decor-envelope-b1 沉淀）

**背景：** decor-envelope-b1 F001 把 acceptance 的 allowed 上沿从双写常量表 `_WALL_BAND_ALLOWED_Z` 改为「渲染顶 + 余量」派生，**余量维持不动** —— 设计上行为应当**完全等价**。这类「提取 / 去重 / 换写法、行为不变」的重构，「测试跑绿」只证明「没崩」，**证明不了「真的只是换了个写法」**（测试可能根本没覆盖到差异点）。

**判据：** 凡声称「行为等价」的重构，先问一句 **「能不能做出逐字节输出对照？」** 能则它是金标准。本案在**生产实物**上重建 allowed mask，pre/post `sha256` 均 `81cdbeea..`、on-px 均 103135 —— 这一条等式比 359 个测试跑绿更有说服力：它直接钉死「输出的每一个 bit 都没变」。用 `git worktree` 检出重构前 commit 与 HEAD **对跑同一份真实输入**取双 sha256。

**配套（阳性对照，防「等价性判据本身失明」）：** 逐字节相同还要证明「代码路径真的换了」（否则可能是两版都走了同一条老分支）。本案额外记录了 mechanism 串（`_WALL_BAND_ALLOWED_Z` → `wall_band_allowed_top_mm`）不同 + 定向变异（把 helper 换回旧双写表，2/2 承重断言在**值**上失败）。**「逐字节相同 + 路径已换 + 变异能红」三者合一，才是完整的等价性证明。**

**同族：** 与 §8（fixture 同源错误）/§10（否定性结论须阳性对照）同一元规律 —— **让「没变化」这个否定性结论可证伪，而不是靠「我没看到差异」。**

**来源：** decor-envelope-b1 F001（Generator 实现 + 隔离 Evaluator 独立复现 sha256 一致）。

---

## 15. 跨会话 / 二手报告传递的测量数字，必须带单位与坐标系，否则会被静默误读（v1.0.7 — decor-envelope-b1 沉淀）

**背景：** decor-envelope-b1 的 backlog 立项文本记「新边缘落在 allowed 包络 **8~12px** 内」，但**没标明**这个 px 是**工作空间**（`_WORK_W=512` 宽）还是**原图**（2048 宽）—— 两者差 **4 倍**。验收/实现若直接采信这个裸数字定余量，会错 4 倍。

**如何定死单位（反证法）：** 若是原图 px（=2~3 work-px），则今天的 100mm 余量（=4.0 work-px）**本就该盖住**溢出 → 与「实际有坏块」的事实**自相矛盾** ⇒ 只能是 work-px。**旁证坐标系混用：** 报告者把 tile 坐标写成 384/512（= 32 work-px × 4 = 原图空间），却把**距离**留在工作空间 —— **一份二手报告里同时混了两个坐标系。**

**处理规则：**
- **产出侧**：任何跨会话传递的像素/毫米/比例测量，**必须**在数字旁标单位与参照系（work-px / 原图-px / mm / 占画幅比），像素尤其要标分辨率空间。
- **消费侧**：拿到无单位的二手测量数字，**不得直接采信**去定阈值/容差；先回到实物用第一性原理反算单位（本案的反证法），或直接在实物上重量。
- Generator 定容差/阈值类常量时，commit / handoff 必须给出**换算依据**（本案：r_live 墙面带 1 work-px = 24.8mm）。

**来源：** decor-envelope-b1 F001（Generator 反证定死单位后发现 backlog 数字单位不明 + 坐标系混用）。
...
| 2026-07-16 | v1.0.7：新增 §14（等价性重构以逐字节输出对照为金标准）、§15（二手测量数字必须带单位/坐标系） | 阅天府 decor-envelope-b1 |
