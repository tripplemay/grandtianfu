...
**红队校准（v1.0 — 勿一刀切砍慢车道）：** 慢车道 = git **同步总线**（跨机器异步交接状态文件）在单机场景确为死重；但两样搭在同一标签上的能力单机也真实、不可连带砍：① **独立会话 evaluator** 是比隔离 subagent **更强**的独立性——它读不到编排者写的 prompt，免疫铁律 12 的"作者污染"风险；② **跨会话 / 抗压缩交接**——多日批次 + 上下文压缩会在同一会话内重现"新读者接手"问题，持久状态文件是唯一可靠续接面。**砍慢车道时只砍"git 同步总线"语义，保留"独立会话隔离 / 跨会话持久"两类。**
