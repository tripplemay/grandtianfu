...
## 结论与判据必须分开标注可靠性（v1.0.5 — calib-z-b1 沉淀）

**「结论对、判据错」是对抗验证的常见失效模式。** calib-z-b1 一批里出现**三次**：

- Evaluator A 主张 `1537e` 该修（结论对），但其判据「窗框右缘 x≈430 vs 照片 428」经复核**落在玻璃正中、根本没有窗框**（判据错）
- Generator 主张 commit 夹带物非活泄漏（结论对），判据却是「跑 5 次没看见泄漏」= **证据不足式论证**（正确论证是结构性的：活泄漏经 API 写路径落盘必戳当次时间戳，不可能重建 4 天前的字节）
- Evaluator B 宣称 A 的 `err=195.7px` 无法复现（其实 A 没错：195.7 是各锚点 err 之**和**，正是被测代码自己的度量；123.9 是 max）

**规则：**

1. 报告中**结论**与**支撑判据**分别标注可靠性；不得因结论正确就默认判据成立
2. 优先采用**不依赖目检数字**的结构性论证。本案终审领衔判据即是：两候选是**精确镜像对且对锚点 err 完全相同** ⇒ 数据对二者无偏好 ⇒ 方向只能由物理定；而修后 11/11 门后候选唯一且 `det=-1`（物理可实现），存量 11 条全 `det=+1`（物理不可实现）—— **不依赖任何一张照片的目检**
3. 不予背书他人数字前，先确认自己算的是**同一个统计量**（sum vs max 之别）

## 「排除 / 不修 / 降级」不是中立选择（v1.0.5 — calib-z-b1 沉淀）

calib-z-b1 fix-round 1 曾以「无法定论」**排除**一条标定不自愈，读起来像保守稳妥；实则是**主动把一台已知物理不可实现的镜像相机留在生产**，且自愈报告打印「相机在地板下方：0 条」**读起来像已干净**（该判据对镜像解本就不适用）。两位 Evaluator 独立推翻该排除，判 **healed 严格优于 excluded**。

**规则：** 任何 skip / exclude / 降级 / soft-watch，必须在报告与 spec 中写明：

- **留下的是什么缺陷**（不是"暂不处理"，而是"主动保留 X"）
- **报告不得让被排除项看起来已解决**（如不得让它继承一个对它不适用的"通过"计数）
...
**⚠ 落盘顺序：先落 `evaluator_feedback`，再写详细报告（v1.0.7 — decor-envelope-b1 沉淀）。** 长验收的 subagent 容易在**收尾最后一步**被 API stream idle timeout 截断。decor-envelope-b1 **两个** evaluator subagent 都栽在这里：第一个连报告都没写就死；第二个写完完整详细报告、**就差把结论落进 progress.json** 时死 —— 结果磁盘上「报告说 PASS，但 `evaluator_feedback` 仍为 null」，状态机卡在 verifying 无法推进。

- **对 Evaluator：** 一旦判定成形，**先**用一次短写落 `evaluator_feedback`（哪怕先落骨架 verdict + per_feature，细节随后补），**再**去写 `docs/test-reports/` 的长报告。把「承重的结论落盘」放在「耗时的叙述」之前，故障截断时至少结论已在盘上。别把所有落盘堆在最后一个动作里。
- **对编排者（判死后的容错）：** 若 subagent 确证死亡（明确 failed / idle 终态 + transcript 停增）而其**详细报告已完整**，可先 `SendMessage` 唤醒它自行补落 `evaluator_feedback`（它的 context 还在，这是最优）；唤不醒时，可从**它自己已落盘的报告**逐字**转录**结论进 `evaluator_feedback` —— **转录 ≠ 改写**（搬的是 Evaluator 写下的判断，不是编排者产生的新判断），且必须在 commit / notes 标注来源与转录事实。这不豁免铁律 12，只是在「结论已由 Evaluator 白纸黑字写死」时把它搬到正确的字段。
