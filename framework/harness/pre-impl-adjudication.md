...
### 4.8 Planner 把「读代码觉得合理」写成 spec 断言（2026-07-16 加，decor-envelope-b1 沉淀）

**错误：** Planner 起草 spec 时，基于**读代码的推测**下断言 —— 如 decor-envelope-b1 spec §2.2 原写「挂画盒 `1000..1400` 在实拍真实毫米下**恰好是合理的**，属借错数字但碰巧不出错」。building 期 Generator 用**生产实物**一量即塌：挂画盒与窗帘同源、都是照抄轴测压扁世界的 `axon` SPECS，只是没错得那么显眼。这个错断言若没被实测撞破，会让 Generator「按合理的东西不动它」，把一个真缺陷放行。

**正确：** spec 里每一条「X 是对的 / 合理的 / 不会出错」断言，Planner 自问 **「这是量出来的，还是读代码推测的？」** —— 推测的**必须**标注为「待验」（如「initial read 下看似合理，F00x 验收须实测确认」），**不得写成定论**。**当有生产实物 / staging 数据可量时，推测不该进 spec**。

**与 §4.1 的区别：** §4.1 是「Planner 凭印象**裁决**」（该读代码没读）；本条是「Planner 读了代码、但把**代码阅读下的推测**当成了**实测结论**」—— 读了代码仍不够，几何/渲染/数值类断言的地面真值在**实物**而非源码里。

**良性范例（同批）：** 该错断言被 Generator building 期实测撞破后，走了**裁决 #4**（stop-the-line → 用户裁决 → 订正 spec 并保留原文作审计轨迹），没有静默改写。这正是 §2.4 状态机配合期望的形态。
