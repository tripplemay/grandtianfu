...
## v1.0.5 — 2026-07-15（calib-z-b1：7 条 learnings 沉淀）

**来源：** 阅天府 calib-z-b1 bug-fix 批次（标定世界 z 轴符号系统性取反 —— 左手系约定与 `+cross(x,y)` 构造冲突）。该批 4 位隔离 Evaluator、fix_rounds=2。用户 2026-07-15 逐条确认。

**变更（技术域 patterns/）：**
- `patterns/testing-env-patterns.md §8`（新增）：**fixture 与被测代码共享同一错误假设 → 两错相消 → 该 bug 永远测不出**。本案根因带着 200+ 全绿测试上线数月，因为测试 fixture `_synth_camera` 自己也是 `det=+1` 的镜像相机，与被测 `calibrate()` 犯同一手性错误。判据：被测对象是**全局假设**（坐标系 / 手性 / 字节序 / 时区 / 编码 / 单位）时，fixture 必须由**独立于被测实现的第一性原理**构造。自查条：「若被测代码与 fixture 犯同一个错，这条测试还会红吗？」
- `patterns/testing-env-patterns.md §9`（新增）：**验证手性/约定类 bug 时，反证脚本本身极易犯同一个错**。§8 的正解（合成真值）自己也踩了坑 —— 某 Evaluator 第一版真值构造在左手系里做叉乘 → 真值相机上下颠倒 → 一度得出「修复后是错的」的**相反结论**。订正法：在已知右手系里 look-at 造真值，再换基进被测约定。判据：**验证 X 类 bug，反证工具不得依赖 X**。旁证：其随机扫描的「13/200 不还原真值」是**它自己的生成器**把锚点造到相机背后 —— 反证工具的 bug 会伪装成被测对象的 bug。

**变更（harness/）：**
- `harness/evaluator.md`（新增两节）：**结论与判据分开标注可靠性** —— 「结论对、判据错」是对抗验证常见失效模式，本批出现 3 次（含 Evaluator 目检数字落在玻璃正中、Generator 用"跑 5 次没看见"这种证据不足式论证、以及误判他人 sum/max 统计量）；优先采用不依赖目检数字的结构性论证。**「排除/不修/降级」不是中立选择** —— 须写明"主动留下的是什么缺陷"，且报告不得让被排除项继承一个对它不适用的"通过"计数。
- `harness/harness-rules.md §独立性铁则`（新增铁则 5）：**不得并发派工同一验收**。判死 subagent 须有**积极证据**（transcript 停止增长 + 无产物 + 明确终态），"我看不到进展"不算。本批编排者误判首个 Evaluator 已死而另起一个 → 两位并发验收争抢 `progress.json`。
- `harness/harness-rules.md §推送前遗漏检查`（新增两坑）：**`git add -A` 会扫入工作区既有脏文件**（违反铁律 10，本批 F002 commit 夹带 4 天前的本地残留，且与其 commit message 自称的"不可能写穿"矛盾）；**`git stash` 是用户的待办队列，不是 agent 的临时空间**（上述夹带物真凶 = 一个标注"待用户决定去留"的 stash 被 pop 且丢弃 = **用户的决定被无声代做**；查实靠 blob 同一性，恢复用 `git stash store`；基线对照请用 `git worktree`）。
